...
Other than the find methods mentioned above, interactions with a job-collection occur using the `JobCollection`, `Job` and `JobQueue` APIs documented below. The Job document model used in the job-collection is fully specified, maintained and enforced by the APIs.
